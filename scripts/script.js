...
// Escondendo os elementos iniciais
hideEverything()

// Configurações para a animação de confetes
const confettiSettings = { target: "confetti" }
const confetti = new window.ConfettiGenerator(confettiSettings)
confetti.render()

// Definindo unidades de tempo
const second = 1000,
  minute = second * 60,
  hour = minute * 60,
  day = hour * 24

// Calculando o tempo restante até a data de nascimento
let countDown = new Date(`${config.birthdate} 00:00:00`).getTime()
x = setInterval(function () {
  let now = new Date().getTime(),
    distance = countDown - now

  // Atualizando a contagem regressiva no HTML
  ...
  // Configurações para o canvas de animação
  let w = (c.width = window.innerWidth),
    ...
    hh = h / 2,
    opts = {
      strings: ["FELIZ", "ANIVERSARIO!", config.name],
      charSize: 30,
      charSpacing: 35,
      lineHeight: 40,
  ...
  // Configurações iniciais para o contexto do canvas
  ctx.font = opts.charSize + "px Verdana"

  // Definição da classe Letter para representar cada letra
  ...
  // Método step para avançar o estado de um fragmento em cada quadro da animação
  ...
  // Inicialização das letras com base nas configurações
  ...
  // Atualização do tamanho do canvas ao redimensionar a janela
  ...
  // Exibição dos elementos HTML com base na contagem regressiva
  if (distance > 0) {
    head.style.display = "initial"
    count.style.display = "initial"
  } else {
    // Exibição do presente e início da animação ao abrir a caixa
    head.style.display = "none"
    count.style.display = "none"
